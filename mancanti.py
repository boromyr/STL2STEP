"""Quali CONFINI fra facce del CAD sfuggono al rilevatore, e perche'."""
import sys, math
import numpy as np
...
coppie = defaultdict(lambda: [0, 0, [], [], []])   # (a,b) -> n, presi, angoli, trame, cos
for i in range(nF):
    for j in topo.adj[i]:
        if j <= i or of[i] == of[j]:
            continue
...
print(f"confini fra facce originali: {len(coppie)}")
righe = []
...
print("  i confini PERSI, dal piu' lungo:")
for f, n, a, b, ang, ds, cd in persi3[:14]:
    print(f"   {tipA[a].replace('GeomAbs_',''):14s} area {arA[a]:7.2f} | "
          ...
          f"{n:4d} spigoli, presi {100*f:5.1f}%  (diedro {ang:6.2f}, trama {ds:5.2f}, cos {cd:5.2f})")
tot = sum(r[1] for r in righe)
ok = sum(r[0] * r[1] for r in righe)
persi = [r for r in righe if r[0] < 0.5]
print(f"  confini persi (meno del 50%): {len(persi)} su {len(righe)}")
